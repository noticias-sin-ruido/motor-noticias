...
- ~~**Amplificación de costo**~~ ✅ **cerrada el 05/09/2026**: `POST /clusters/{id}/synthesize` exige material nuevo salvo `?forzar=true`. N llamadas idénticas pasaron de N a 1 llamada al proveedor. Detalle en `change_logs.md`. **Quedan sus dos efectos colaterales**, que son decisiones aparte:
  - **El reset de `enviado_backend`/`intentos_envio`** en cada re-síntesis es deliberado y está bien fundado (el cuerpo cambió, merece su oportunidad), pero **nadie evaluó el efecto combinado**: repetir la re-síntesis reinicia el contador, así que `WEBHOOK_MAX_INTENTOS` deja de ser un techo real para una síntesis que se re-sintetiza seguido. La guarda nueva lo acota —ahora hace falta `forzar=true`— pero no lo cierra.
  - **No mira el estado del cluster**: uno `descartado` —cerrado por no alcanzar el mínimo de dos medios, que es la regla que define el producto— se sintetiza igual y queda listo para entregar. Hay una defensa parcial río abajo (`_persistir` descarta ángulos que no cubran `MIN_MEDIOS_CLUSTER`), pero es sobre el ángulo y depende de la comparativa que escriba el modelo, no de un invariante del motor.
